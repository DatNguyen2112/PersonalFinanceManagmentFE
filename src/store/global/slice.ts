...
import type { Responses } from "../../models/api";
...
export type ProfileUser = User & Auth["user"];

const hasProfileFields = (value: unknown): value is ProfileUser => {
  if (!value || typeof value !== "object") return false;
  const user = value as ProfileUser;
  return !!(user.id || user.email || user.userName || user.username);
};

/** Normalize `/auth/me` response — API may return user at root, in `data`, or in `user`. */
export const normalizeAuthMeResponse = (result: unknown): ProfileUser | null => {
  if (!result || typeof result !== "object") return null;

  const response = result as Responses<User> & ProfileUser;

  if (hasProfileFields(response.data)) return response.data as ProfileUser;
  if (hasProfileFields(response.user)) return response.user as ProfileUser;
  if (hasProfileFields(response)) return response as ProfileUser;

  return null;
};

export const getProfileUser = (state: GlobalState): ProfileUser | undefined => {
  const data = state.data as ProfileUser | undefined;
  if (data?.id || data?.email || data?.userName || data?.username) {
    return data;
  }

  const user = state.user;
  if (user && "user" in user && user.user) {
    return user.user as ProfileUser;
  }

  const flatUser = user as User | undefined;
  if (flatUser?.id || flatUser?.email || flatUser?.userName) {
    return flatUser as ProfileUser;
  }
...
export const globalAction = {
  ...
  profile: createAsyncThunk(
    name + "/profile",
    async () => {
      const result = await API.get<User>(`${routerLinks(name, "api")}/me`);
      const profile = normalizeAuthMeResponse(result);
      if (!profile) {
    ...
    },
    {
      condition: (_, { getState }) => {
        const state = (getState() as Record<string, GlobalState>)[name];
        if (state?.status === EStatusGlobal.profilePending) return false;
        if (
          state?.status === EStatusGlobal.profileFulfilled &&
          hasProfileFields(getProfileUser(state))
        ) {
          return false;
        }
        return true;
  ...
  changePasswordProfile: createAsyncThunk(
    name + "/changePasswordProfile",
    async (values: User) => {
      const result = await API.put<{ user: User }>(
        `/idm/users/${values.id}/password`,
        values,
      );
  ...
  login: createAsyncThunk(name + "/login", async (values: LoginModel) => {
    ...
    if (result.isSuccess) {
      ...
      customMessage.error({
        content: result?.errorMessage || "Đăng nhập thất bại",
      });
    }
    return result;
  }),
  register: createAsyncThunk(
    ...
    async (values: RegisterModel) => {
      ...
      if (result.isSuccess) {
        ...
        customMessage.error({
          content: result?.errorMessage || "Đăng ký thất bại",
        });
      }
      return result;
    },
  ),
  forgottenPassword: createAsyncThunk(
    name + "/forgotten-password",
    async (values: { email: string }) => {
      const { message } = await API.post(
        `${routerLinks(name, "api")}/forgotten-password`,
        values,
      );
  ...
  otpConfirmation: createAsyncThunk(
    name + "/otp-confirmation",
    async (values: { email: string; otp: string }) => {
      const { message } = await API.post(
        `${routerLinks(name, "api")}/otp-confirmation`,
        values,
      );
      if (message) await customMessage.success({ content: message });
      return true;
  ...
  extraReducers: (builder) => {
    builder
      .addCase(
        ...
        (state, action: PayloadAction<GlobalState>) => {
      ...
      .addCase(globalAction.logout.fulfilled, (state: any) => {
        ...
        state.status = EStatusGlobal.idle;
      })
      ...
      .addCase(
        globalAction.profile.fulfilled,
        (state: any, action: PayloadAction<ProfileUser>) => {
          if (hasProfileFields(action.payload)) {
      ...
      .addCase(
        globalAction.changePasswordProfile.pending,
        (state: any, action) => {
      ...
      .addCase(
        globalAction.changePasswordProfile.fulfilled,
      ...
      .addCase(
        globalAction.register.fulfilled,
        (state: any, action: PayloadAction<any>) => {
      ...
      .addCase(
        globalAction.forgottenPassword.fulfilled,
      ...
      .addCase(
        globalAction.otpConfirmation.fulfilled,
        (state, action: PayloadAction<boolean>) => {
          if (action.payload)
      ...
      .addCase(
        globalAction.resetPassword.fulfilled,
