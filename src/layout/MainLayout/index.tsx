import {
    LayoutDashboard, ArrowLeftRight, PieChart, BarChart2, Wallet,
...
const navItems: NavItem[] = [
    { path: routerLinks('Dashboard'), label: 'Tổng quan', icon: <LayoutDashboard size={20} /> },
    { path: routerLinks('Transactions'), label: 'Giao dịch', icon: <ArrowLeftRight size={20} /> },
    { path: routerLinks('Budget'), label: 'Ngân sách', icon: <PieChart size={20} /> },
    { path: routerLinks('Reports'), label: 'Báo cáo', icon: <BarChart2 size={20} /> },
    { path: routerLinks('Wallets'), label: 'Ví tiền', icon: <Wallet size={20} /> },
];
...
export default function Layout({ onLogout, userEmail }: Props) {
        ...
        `}
            >
                ...
                <div className="flex items-center gap-3 px-6 py-5 border-b border-slate-700/50">
                    ...
                    <div>
                        <h1 className="text-white font-semibold text-sm leading-tight">FinanceApp</h1>
                ...
                <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                    <p className="text-slate-500 text-xs font-medium uppercase tracking-wider px-3 mb-3">Menu</p>
                    {navItems.map(item => {
                ...
                <div className="px-3 py-4 border-t border-slate-700/50 relative">
                    ...
                    >
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-slate-200 text-sm font-medium truncate">{currentUserEmail.split('@')[0]}</p>
                            <p className="text-slate-500 text-xs truncate">{currentUserEmail}</p>
                        </div>
                        <ChevronRight size={14} className={`text-slate-400 transition-transform ${showProfile ? 'rotate-90' : ''}`} />
                    </button>

                    {showProfile && (
                        <button
                            onClick={async () => { setShowProfile(false); await handleLogout(); }}
                            className="w-full mt-1.5 flex items-center gap-2 px-3 py-2 text-slate-300 hover:text-slate-100 hover:bg-slate-800 rounded-lg text-xs font-medium transition-colors"
                        >
                            <LogOut size={14} />
                            Đăng xuất
                        </button>
                ...
                <header className="bg-white border-b border-slate-100 px-4 lg:px-8 py-4 flex items-center gap-4 flex-shrink-0">
                    ...
                    <div>
                        <h2 className="text-slate-800 font-semibold text-lg leading-tight">
                            {navItems.find(n => location.pathname === n.path || (n.path !== '/' && location.pathname.startsWith(n.path + '/')))?.label || 'Tổng quan'}
                        </h2>
